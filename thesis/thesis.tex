...
\usepackage{varioref} % References with page numbers (if you want that, use \vref instead of \ref)
...
% for the Euro symbol (\euro{}) or currency amounts: \EUR{47,11}
\usepackage{eurosym}

% \usepackage{floatflt} % floating figures (Figures/Tables, surrounded by text, most of the time a bad idea)
...
\usepackage{url}
%\usepackage{hyperref} % clickable links with funny color boxes... better use the config below
...
\usepackage[square]{natbib} % use the natbib library package, see docs and the NICE TO KNOW section below! We use square brackets.
...
%\hyphenation{My-word-which-shall-always-only-be-seperated-like-this and-another-word and-a-third-one and-so-on} % if you spot mis-hyphenations, insert them here.
...
\newcommand{\supervisorA}{Prof. Dr. My Example Prof}
\newcommand{\supervisorB}{Dr. My Example Dr}
\newcommand{\submissiondate}{March\ 5th, 2012} % English date (same as German for German thesis)
\newcommand{\submissiondateDE}{5. März 2012} % German date (used in "erklaerung.tex")
...
\newcommand{\lastAccessed}{(last accessed: Mar.\ 3rd, 2012)} % e.g., for URIs
...
%\nocite{*} % if you want to include everything from your library even if never \cited (usually a bad idea)
...
% a file called bibFrozen.tex and include that. By that your thesis lib won't
